...
VCModelBoth <- lmer(VC+0.01 ~ Class * Block * Calculation + (1|Site_ID),
                     data = VecCapacity[VecCapacity$VC>0,]) 

summary(VCModelBoth)
tidy(VCModelBoth)

VCpreds <- cbind(VecCapacity[VecCapacity$VC>0,], predict(VCModelBoth, response = T))
```

Because there is no significant difference, I only report the other stats on the field-based calculation.

```{r}
#gamma has better residuals than gaussian
...
```{r}
VCTempModel <- lm(sqrt(VC+0.01) ~ meanT, 

...
infLong <- seasonInf %>%
  ...
  summarise(mean.inf=mean(infection, na.rm=T), sampleSize=sum(!is.na(infection))) %>%
  group_by(block, class, type) %>%
  summarise(se.inf=sd(mean.inf, na.rm=T)/n(), mean.inf=mean(mean.inf), samples=sum(sampleSize)) %>%
  ungroup()
infLong$sampleLab <- paste0("n=", infLong$samples)
```
...
#png(file="figures/forMS/landclassXseason.png", width = 4, height=7, units="in", res=600, family="sans")
ggplot(data=infLong[order(infLong$block, decreasing=F),], aes(x=class, group=block))+
  ...
  geom_text(aes(label=samples, y=mean.inf), vjust=-1.7, color="black", position=position_dodge(width=-0.9), size=3)+
  facet_wrap(~factor(type), nrow=3, dir="v", scales='free') +
  ...
  guides(fill=F, color= F,
         alpha=guide_legend(override.aes=list(color="gray40"), reverse=T)) +

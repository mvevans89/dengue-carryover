...
ggplot(data=infLong[order(infLong$block, decreasing=F),], aes(x=class, group=block))+
  geom_bar(stat="identity", aes(y=mean.inf, alpha=factor(block), fill=factor(class)), color="gray40", position=position_dodge(width=-0.9))+
  facet_wrap(~type, nrow=3, dir="v") +
  ylim(0,1)+
  scale_fill_manual(values=c(colR, colS, colU))+
  scale_alpha_discrete(range=c(0.3,1), name="Season", labels=c("Fall", "Summer"), guide=guide_legend(reverse=T))+
  geom_errorbar(aes(ymin=mean.inf-se.inf, ymax=mean.inf+se.inf), width=0.2, color="gray40", position =position_dodge(-0.9))+
  xlab("Land Class") +
...
ggplot(data=VCplot, aes(x=Temp_mean_mean))+
  ...
  scale_colour_manual(name="Calcuation Type", 
                      values=c(WithCOE="#f1a340", WithoutCOE="#998ec3"),

...
ggplot(data=VCplot, aes(x=Temp_mean_mean))+
  ...
  geom_smooth(aes(y=VC2, color="WithoutCOE"),  method="lm", show.legend=F)+
  geom_smooth(aes(y=VC, color="WithCOE"), method="lm", fill=NA)+
  geom_smooth(aes(y=VC2, color="WithoutCOE"),  method="lm", fill=NA)+
...
VCplotsumm <- VCplot %>%
  ...
  gather(calc, value, VC, VC2, VCdiff) %>%
  group_by(block, Class, calc) %>%
  dplyr::summarise_each(funs(mean=mean(., na.rm=T), se=sd(.)/sqrt(n()))) %>%
  ungroup()
#add in facet labels
VCplotsumm$labels <- case_when(
  VCplotsumm$calc=="VC" ~ "Vectorial Capacity",
  VCplotsumm$calc=="VC2" ~ "Vectorial Capacity",
  VCplotsumm$calc=="VCdiff" ~ "Change in Vectorial Capacity"
)
```
...
ggplot(data=VCplotsumm, aes(x=factor(Class), group=block)) +
 ...
 theme(strip.text.x = element_blank()) #this has to be seperate for some reason?
dev.off()
```

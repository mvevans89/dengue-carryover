...
finalBody <- lm(Body_mean~ exp(Temp_mean_mean), data=modDF)
plot(finalBody)
car::Anova(finalBody)
confint(finalBody)
summary(finalBody)
...
finalHead <- lm(Head_mean~ exp(Temp_mean_mean), data=modDF)
Anova(finalHead)
plot(finalHead)
```
...
```{r}
wingMix <- lmer(mm~0 +block+class +(1|site),
                data=allWing)
summary(wingMix)
plot(wingMix)
car::Anova(wingMix)
...
```{r}
anova(lm(r~Class*block, data=growthDF2)) #only block is significant
plot(lm(r~Class*block, data=growthDF2))
```

#Vectorial capacity using Ross-McDonald equation

I will calculate the vectorial capacity with the Ross-McDonald equation. 

The per capita growth rate will be used as a proxy of mosquito density. Bite rate, EIP, and adult survival will be calculated using rate summation from Mordecai et al 2017.  Vector competence will be a direct measure by site from our empirical work.
...
paramRate$Class <- as.factor(paramRate$Class)
```

Combine these calculated parameters with field derived growth and vector competence:

```{r}
growthSite <- growthDF2 %>%
  dplyr::group_by(block, Class, Site_Code) %>%
  dplyr::summarise(rCap=mean(r, na.rm=T))
```

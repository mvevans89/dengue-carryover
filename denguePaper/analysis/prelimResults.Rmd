...
paramNoCOE$fecundity2 <- ((4.88/100)*paramNoCOE$Temp*(paramNoCOE$Temp-8.02)*((35.65-paramNoCOE$Temp)^0.5))/24
...
# paramNoCOE$pEA <- ((-3.61/1000)* (paramNoCOE$Temp-9.04)*(paramNoCOE$Temp-39.33))/24
# paramNoCOE$pEA[paramNoCOE$pEA<0] <- 0
# paramNoCOE$pEA[is.na(paramNoCOE$pEA)] <- 0
# 
# paramNoCOE$MDR <- ((6.38/100000)*paramNoCOE$Temp*(paramNoCOE$Temp-8.60)*((39.66-paramNoCOE$Temp)^0.5))/24
# paramNoCOE$MDR[paramNoCOE$MDR<0] <- 0
# paramNoCOE$MDR[is.na(paramNoCOE$MDR)] <- 0

paramNoCOE$bc2 <- (((7.35/10000)*paramNoCOE$Temp*(paramNoCOE$Temp-15.84)*((36.40-paramNoCOE$Temp)^0.5)) * ((4.39/10000)*paramNoCOE$Temp*(paramNoCOE$Temp-3.62)*((36.82-paramNoCOE$Temp)^0.5)))/24
...
```{r}
paramRate$block <- as.factor(paramRate$block)
levels(paramRate$block) <- c("Fall","Summer")
#png(file="figures/forMS/relativeR0.png", width = 6, height=4, units="in", res=500, family="sans")
ggplot(data=paramRate, aes(x=factor(Class), y=R0))+
  #geom_boxplot(aes(color=Class))+
  geom_point(aes(color=Class), size=3) +
  facet_wrap(~factor(block))+
  theme_fivethirtyeight() +
  ...
  ylab("Relative R0")+
  xlab("Land Class") +  
  scale_x_discrete(name=c("Rural", "Suburban", "Urban"))+
  theme(legend.position="right", legend.direction="vertical")+
  guides(fill=F, color=F) 
#dev.off()
```
...
VCplotsumm <- VCplot %>%
  group_by(block, Class) %>%
  dplyr::summarise(meandiff=mean(VCdiff, na.rm=T), sediff=sd(VCdiff)/sqrt(n()))
ggplot(data=VCplotsumm, aes(x=factor(Class), group=block)) +
  geom_bar(stat='identity',aes(y=meandiff, fill=Class, alpha=block), color="black", position=position_dodge(0.9)) + 
  geom_errorbar(aes(ymin=meandiff-sediff, ymax=meandiff+sediff), position=position_dodge(0.9), width=0.4)+
  theme_base()+
  ylab("Percent Change in VC due to COEs")+

...
We found that infection in the body and head was significantly lower in the summer than the fall. Although there was a trend towards higher infection in the saliva in the summer, this was not significant (p=0.05676).  Land class also influenced infection of both the body and head, with urban mosquitoes having lower rates of infection than rural and suburban mosquitoes. Saliva infections were unaffected by land class, however.
...
knitr::kable(Saliva21Aov, caption="Mixed model Wald Test for Saliva infections.")
```
...
#curve(4.6320-1.8761*x, 1.5, 3, add=T, lwd=2)
#with(bodyWingDF, abline(lm(Body~Wing)))
ggplot(data=bodyWingDF, aes(x=factor(Body), y=Wing))+
  geom_boxplot() +
  geom_jitter(shape=16, position=position_jitter(0.3), aes(color=factor(class))) +
...
#with(headWingDF,plot(x=jitter(Wing), y=Head, pch=c(16,17)[block], col=c(colR, colS, colU)[class]))
#curve(4.6320-1.8761*x, 1.5, 3, add=T, lwd=2)
#with(bodyWingDF, abline(lm(Body~Wing)))
ggplot(data=seasonInf, aes(x=factor(Head), y=Wing))+
  geom_violin()

knitr::kable(Anova(headWing))
```

No significant effect of body size on head infection.
...
with(salWingDF,plot(x=jitter(Wing), y=Saliva, pch=c(16,17)[block], col=c(colR, colS, colU)[class]))
#curve(4.6320-1.8761*x, 1.5, 3, add=T, lwd=2)
#with(bodyWingDF, abline(lm(Body~Wing)))
ggplot(data=wingInf, aes(x=factor(Saliva), y=Wing))+
  geom_violin()

knitr::kable(Anova(salWing))
```
...
ggplot(data=modDF, aes(y=Temp_mean_mean, x=RH_mean_mean))+ #can include RH if I want becuase corr=0.74 and vif is good
  geom_point(aes(size=Body_mean, color=class), pch=1)+
  theme_base()

ggplot(data=modDF, aes(x=log(Temp_mean_mean), y=Body_mean))+
  geom_point()+
  geom_smooth(method="lm")

ggplot(data=modDF, aes(x=RH_mean_mean, y=Body_mean))+
  geom_point()

#modDF$scaleTemp <- scale(modDF$Temp_mean_mean, center=T, scale = F)
...
ggplot(data=seasonInfSite, aes(x=Temp_mean_mean, y=Body_mean))+
  ...
  theme(legend.position="right", legend.direction="vertical") +
  stat_function(fun=function(Temp_mean_mean) 1.31354 - 0.03624*(Temp_mean_mean))
#dev.off()
```
...
finalHead <- lm(exp(Head_mean)~ Temp_mean_mean, data=modDF)
Anova(finalHead)
plot(finalHead)
summary(finalHead)
```
...
finalSal <- lm(Saliva_mean~ Temp_mean_mean, data=modDF)
Anova(finalSal)
plot(finalSal)
summary(finalSal)
```

Three panel plot of infection dynamics and temperature
```{r}
  
#png(file="../figures/forMS/InfxTemp3Panel.png", width = 4, height=7, units="in", res=500, family="sans")
...
km.fit <- survival::survfit(Surv(Exp_Day, event) ~ block + Site_Code, data=allSurv, type="kaplan-meier", se.fit=T, conf.int=0.95)
...
#png(file="figures/forMS/emergence.png", width = 6, height=4, units="in", res=700)  
  ggplot(data=d, aes(x=time, y=emerge, group=factor(strata)))+
    ...
    #xlim(0,45)+
    guides(color=F)+
    annotate("text", x=12, y=70, label="Summer")+
    annotate("text", x=25, y=42, label="Fall")+
...
mixEmerge <- lmer(Exp_Day ~  0 + block + Class + (1|Site_Code), 
                 data=emergExpF
                   )
plot(mixEmerge)
...
The day of emergence differed significantly across block. Mosquitoes in the summer emerged on average 12 days sooner than those in the fall. Urban mosquitoes developed more quickly, but not significantly.

## Temperature

Because this data is calculated at a tray level, we will tie in tray-level climate data.
...
```{r}
ggplot(data=survClim, aes(x=Temp_mean, y=percSurv))+
  geom_point(aes(color=Class, shape=block), size=3) +
  theme_fivethirtyeight()+
  theme(axis.title = element_text()) +
  theme(panel.background = element_rect(fill = "transparent", colour = NA), 
        plot.background = element_rect(fill = "transparent", colour = NA),
  ...
  scale_color_manual(values=c("dodgerblue", "gray10", "maroon"), labels=c("Rural", "Suburban", "Urban"))+
    scale_fill_manual(values=c("dodgerblue", "gray80", "maroon"), labels=c("Rural", "Suburban", "Urban")) +
  ylab("Number Females Survived (Out of 50)") +
...
```{r}
survClimMod <- lm(percSurv~Temp_mean, data=survClim)
ggplot(data=survClim, aes(y=percSurv, x=Temp_mean))+
  geom_point()+
  geom_smooth(method="lm")+
  facet_wrap(~block)
survClimModmix <- lmer(log(percSurv)~Temp_mean + (1|Site_Code),
                    data=survClim)
...
Now we look at temperature and emergence day.
...
```{r}
ggplot(data=emergClim, aes(x=Temp_mean, y=devRate))+
  geom_point(aes(color=Class, shape=block), size=3) +
  theme_fivethirtyeight()+
  theme(axis.title = element_text()) +
  theme(panel.background = element_rect(fill = "transparent", colour = NA), 
...
```{r}
survClimMod <- lm(devRate~Temp_mean,
                    data=emergClim)
...
The growth rate is calculated using the Livdahl Stringer Equation.

Because I do not have the average wing length per day of emergence for all emergence events, I will use an average per tray for those that emerged prior to when we began mounting wings.
...
Then calculate the per capita growth rate:
...
#Vectorial capacity using Mordecai et al 2017

I will calculate the vectorial capacity with the equation from Mordecai et al. 2017:

$$ VC(T) = \left (\frac{a(T)^2b(T)c(T)e^{-\mu (T)/PDR(T)}EFD(T)p_{EA}(T)MDR(T)} {\mu(T)^3}  \right ) ^{1/2} $$

Bite rate, EIP, and adult survival will be calculated using rate summation from Mordecai et al 2017.  Vector competence, eggs per female per day, egg to adult survival probability and mosquito immature development rate are empirically measured. 

```{r}
#from cleaned climate
#subset out dates I want
...
Calculated with environmental temperatures:

```{r, eval=F}
#a -- bite rate
parameters$a <- ((1.93/10000)*parameters$Temp*(parameters$Temp-10.25)*((38.32-parameters$Temp)^0.5))/24
#adjust negatives
parameters$a[parameters$a<0] <- 0
parameters$a[is.na(parameters$a)] <- 0

# PDR - parasite development rate
parameters$PDR <- ((1.09/10000)*parameters$Temp*(parameters$Temp-10.39)*((43.05-parameters$Temp)^0.5))/24
#adjust negatives
parameters$PDR[parameters$PDR<0] <- 0
parameters$PDR[is.na(parameters$PDR)] <- 0

# L - mosquito lifespan
parameters$L <- (-1.43*(parameters$Temp-13.41)*(parameters$Temp-31.51))/24
#adjust for zeros
parameters$L[parameters$L<0] <- 0
parameters$L[is.na(parameters$L)] <- 0
...
paramRate$mu <- 1/paramRate$L

## EIP -
#paramRate$EIP <- 1/paramRate$PDR
...
Combine these calculated parameters with field EFD, survival, and emergence rates and vector competence:
...
Merge all of the estimated and measured traits together
...
Calculate EFD with total eggs and gonotrophic cycle duration (assumed to be inverse of bite rate re: Mordecai et al.)
```{r}
paramRate$EFD <- paramRate$fecundity*paramRate$a
```

###Calculate VC w/o carry over effects
...
#pEA and MDR are not included because they are direct effects of microclimate, not COEs
# paramNoCOE$pEA <- ((-3.61/1000)* (27-9.04)*(27-39.33))/24
# paramNoCOE$pEA[paramNoCOE$pEA<0] <- 0
# paramNoCOE$pEA[is.na(paramNoCOE$pEA)] <- 0
# 
# paramNoCOE$MDR <- ((6.38/100000)*27*(27-8.60)*((39.66-27)^0.5))/24
# paramNoCOE$MDR[paramNoCOE$MDR<0] <- 0
# paramNoCOE$MDR[is.na(paramNoCOE$MDR)] <- 0
...
Merge together with other traits
...
#make data long
# VCplot <- paramAll %>%
#   select(block, Class, Site_Code, VC, VC2) %>% #VC2 is without COE
#   gather(COE, value, -block,-Class,-Site_Code)
#or use the difference
VCplot <- paramAll %>%
  select(block, Class, Site_Code, VC, VC2) %>%
  mutate(VCdiff=(VC-VC2)/VC2*100)
VCplot$block <- as.factor(VCplot$block)
levels(VCplot$block) <- c("Fall", "Summer")
levels(VCplot$block) <- levels(VCplot$block)[c(2,1)]
...
plotNoCOE <- ggplot(data=VCplotsumm, aes(x=factor(Class), group=block)) +
  geom_bar(stat='identity',aes(y=VC2_mean, fill=Class, alpha=block), color="black", position=position_dodge(0.9)) + 
...
plotCOE <- ggplot(data=VCplotsumm, aes(x=factor(Class), group=block)) +
  geom_bar(stat='identity',aes(y=VC_mean, fill=Class, alpha=block), color="black", position=position_dodge(0.9)) + 
...
plotDiff <- ggplot(data=VCplotsumm, aes(x=factor(Class), group=block)) +
  geom_bar(stat='identity',aes(y=VCdiff_mean, fill=Class, alpha=block), color="black", position=position_dodge(0.9)) + 
  #geom_errorbar(aes(ymin=meandiff-sediff, ymax=meandiff+sediff), position=position_dodge(0.9), width=0.4)+
  theme_base()+
  ylab("Percent Change in VC")+
  #ggtitle("Effect of COEs on vectorial capacity") +
  xlab("Class")+
  scale_x_discrete(labels=c("Rural", "Suburban", "Urban"))+
  scale_fill_manual(values=c(colR, colS, colU), labels=c("Rural", "Suburban", "Urban")) +
  scale_alpha_discrete(range=c(1,0.3), name="Season", guide=guide_legend())+
  labs(fill=c("Class","Rural", "Suburban", "Urban"))+
...
pgrid <- plot_grid(plotNoCOE, plotCOE, plotDiff, plotLegend, ncol=4, rel_widths = c(1,1,1,0.4))
...
```{r}
r0mod <- lm(R0~block, data=paramRate)
plot(r0mod)
summary(r0mod)
anova(r0mod)
```

There was a significant difference across season, but not land class.

Does including COEs make a difference for VC?

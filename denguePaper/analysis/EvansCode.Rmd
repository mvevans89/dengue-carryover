...
#pairwise
summary(multcomp::glht(mixEmerge, linfct = multcomp::mcp(block = "Tukey"), test = adjusted("holm")))

summary(multcomp::glht(mixEmerge, linfct = multcomp::mcp(Class = "Tukey"), test = adjusted("holm")))

emergeGrid <-lsmeans::ref.grid(mixEmerge)
emergePair <- pairs(lsmeans::lsmeans(mixEmerge, ~Class|block))
lsmeans::lsmeans(mixEmerge, ~block)
```

Land use trends were significant in the fall:
```{r}
mixEmergeFall <- lmer(Exp_Day ~ Class + (1|Tray_Code)+(1|Site_Code), 

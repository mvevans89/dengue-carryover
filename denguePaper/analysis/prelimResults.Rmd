...
To calculate the vectorial capacity, I will use a method similar to that used in Murdock et al. 2017.

I will calculate parameters by hour by site for the VC equation and propagate uncertainty through MCMC methods.
...
We will need to combine this with the mosquito development rate and survival per site.


Calculate parameters based on equations from Mordecai et al 2017.
...
# we will then calculate the mean and se for these parameters per site and hour
paramRate <- parameters %>%
  group_by(Block, Class, Site_ID, Hour) %>%
  select(-Temp, -Day,) %>%
  summarise_each(funs(mean(.,na.rm=T), sd=sd())) %>%
  ungroup()

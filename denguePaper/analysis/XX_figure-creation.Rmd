...
VecCapacityBlock <- VecCapacityClimate %>%
  ...
  summarise(VC.mean = mean(VC), VC.se = sd(VC), meanT = mean (meanT), VC.median = median(VC)) %>%
  ungroup() %>%
...
```{r}
comparePolygons <- range.box(x = VecCapacityBlock$offset[1], 
                  y.mean = VecCapacityBlock$VC.mean[1],
                  y.sd = VecCapacityBlock$VC.se[1],
                  id = 1,
                  Calculation = VecCapacityBlock$Calculation[1])
for (i in 2:nrow(VecCapacityBlock)){
  tmp <- range.box(x = VecCapacityBlock$offset[i], 
                  y.mean = VecCapacityBlock$VC.mean[i],
                  y.sd = VecCapacityBlock$VC.se[i],
                  id = i,
                  Calculation = VecCapacityBlock$Calculation[i])
  comparePolygons<- rbind(comparePolygons, tmp)
}
rm(tmp)
```
...
```{r}
ggplot() +
  geom_polygon(data=comparePolygons, aes(x = x, y = y, group = id, fill = Calculation), alpha = 0.3, color = "gray10") +
  geom_errorbar(data = VecCapacityBlock, aes(x=offset, ymin = VC.median, ymax = VC.median), color = "gray10")+
  #geom_point(data = VecCapacityBlock, aes(x=offset, y = VC.mean), shape = "-", size=15, alpha = 0.8) +
  geom_point(data = VecCapacityClimate, aes(x=meanT, y = VC, color = Class, shape = Calculation), 
             size = 3) +
  ...
  scale_fill_manual(values = c("black", "gray70"),
                     labels = c("Field Based", "Grand Mean"))
```

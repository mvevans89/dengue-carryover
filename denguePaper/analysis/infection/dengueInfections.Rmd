...
- lmer model: informed by variable selection from glmnet. It is informed in that all of the variables are extremely correlated, but mean_temp came out as important. And I think it makes the most sense to focus on that for now.
...
ggpairs(seasonInfSite, 8:16,lower = list(continuous = line_fn))
```
...
rsquared(list(lmerMod2, lmerMod3, lmerMod4, lmerMod5, lmerMod6)) #lmerMod5 is the best for Rsquared and AIC
...
#check predictive R2
preds <- predict(lmerMod5)
plot(preds, modDF$Body_mean)
abline(a=0, b=1)
```

Based on this, I think I will choose to use an lmer. This is reasonable becaause the relationship does seem to be linear, especially once the block is set as a random effect.

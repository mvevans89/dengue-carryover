...
I'm on the fence about boxplots, so here are site level means to try instead.
...
demographicPlot <- ggplot(data = allDemoSite, aes(x = as.factor(Class), y = mean)) +
  ...
  geom_pointrange(aes(color = Class, shape = Block, group = Block, ymin = mean-se, ymax = mean+se), position = position_jitterdodge(dodge.width = 0.75, jitter.width = 0.2), size = 1) +
  ...
  guides(shape = F, alpha = F, color = F) +
  xlab("Land Class") +
  scale_color_manual(values=c(colR, colS, colU), labels=c("Rural", "Suburban", "Urban"))+
  theme_base() +
  theme(
...
This plot looks bad so I will try to facet it instead.

```{r}
pdf(file="denguePaper/results/figures/forMS/infectionFacet.pdf", width = 3.25, height=6, family="sans")

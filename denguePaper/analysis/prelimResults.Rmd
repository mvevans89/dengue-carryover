...
finalBody <- lm(exp(Body_mean)~ Temp_mean_mean, data=modDF)
plot(finalBody)
car::Anova(finalBody)
confint(finalBody)
summary(finalBody)
with(modDF,plot(y=exp(Body_mean), x=Temp_mean_mean))
```
...
ggplot(data=seasonInfSite, aes(x=Temp_mean_mean, y=Body_mean))+
  ...
  theme(legend.position="right", legend.direction="vertical") +
  stat_function(fun=function(Temp_mean_mean) 0.6038 - (9.646*10^(-13))*exp(Temp_mean_mean))
#dev.off()
```
...
#Vectorial capacity using Ross-McDonald equation
...
```{r}
params2 <- merge(paramRate, growthSite, by.x=c("Block", "Class", "Site_ID"), by.y=c("block", "Class", "Site_Code"))
levels(seasonInfSite$block) <- c("Fall", "Summer") 
params3 <- merge(params2, seasonInfSite, by.x=c("Block", "Class", "Site_ID"),
                 by.y=c("block", "class", "site"))
#add in survival if I want to use that instead of rCap
siteSurv<- survSumm %>%
  group_by(block, Class, Site_Code) %>%
  dplyr::summarise(surv=mean(percSurv, na.rm=T)/50)
params4 <- merge()
siteVC <- params3 %>%
  select(Block, Class, Site_ID, a, EIP, mu, rCap, Saliva_mean) %>%
  mutate(VC=(rCap*(a^2)*Saliva_mean*mu^EIP)/(-log(mu)))
```

Plot:

```{r}
siteVC$Block <- factor(siteVC$Block, levels=c("Summer", "Fall"))
ggplot(data=siteVC, aes(x=Class, y=VC))+
  geom_boxplot()+
  facet_wrap(~Block)+
  theme_fivethirtyeight() +
  theme(panel.background = element_rect(fill = "transparent", colour = NA), 
...
```{r}
anova(lm(VC~Class*Block, data=siteVC))
plot(lm(VC~Class*Block, data=siteVC))
```

...
```{r load data}
emergence <- read.csv("../data/emergence.csv")
```
...
#feed date 1: 7-17-2016: R2, U2, S1, S2
#feed date 2: 7-20-2016: R1, R3, U1, U3, S3
for (i in 1:nrow(females)){
  if (females$Site[i] %in% c(2, 7, 3, 5)) {
  females$age[i] <- as.numeric(format(as.Date("2016-07-17"), "%d"))-females$Day[i]
  } else females$age[i] <- as.numeric(format(as.Date("2016-07-20"), "%d"))-females$Day[i]
}
...
```{r all emergence data}

othEmerge <- read.csv("../data/emergence_nonInfections.csv")
allEmerge <- rbind(emergence, othEmerge)
```

We can similarly look at the total emergence by tray:

```{r emergence by tray ALL}
#cumulative emergence
byTrayAll <- allEmerge %>%
  group_by(Class, Tray_Code, Sex) %>%
  summarise(cumulative=sum(Num_Emerge)) %>%
...
```{r}
print(byTrayAllMF[order(byTrayAllMF$cumulative, decreasing=T),])
```

#The Slow Down

It is also helpful to see when the last mosquito emerged from each tray, to know when the experiment is over. The following is a list of the trays which have not had a mosquito emerge for seven days, and the last day of emergence for them:
...
trayLastDay[(as.numeric((format(Sys.Date(), "%d")))-trayLastDay$lastDay)>=7,]
```
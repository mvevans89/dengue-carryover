...
Relative Humidity
```{r}
#png(file="../figures/forMS/supplement/seasonRH.png", width = 6, height=4, units="in", res=500, family="sans")
ggplot(data=clim, aes(x=Day, group=Class))+
...
```{r}
## ------ mean RH
climModMeanRH <- lmer(meanRH~0 + Class*Block+ (1|Day) + (1|Site_ID), data=climTray)
plot(climModMeanRH)
...
climModMinRH <- lmer(minRH~0+Class*Block + (1|Day) + (1|Site_ID), data=climTray)
plot(climModMinRH)
...
climModDHR <- lmer(DHR~0+Class*Block + (1|Day)+ (1|Site_ID), data=climTray)
plot(climModDHR)
summary(climModDHR)
confint(climModDHR)

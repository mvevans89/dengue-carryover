...
```{r efficiency plot function, eval=F}
...
```{r seasonal 21 dpi Saliva infection}
#explore data
ggplot(data=seasons[seasons$DPI=="21",], aes(factor(class))) +
  geom_bar(stat="identity",aes(y=Saliva)) +
  facet_wrap(~block) + 
...
```{r august climate formatting, message=T}
...
There are some variables that are really correlated. 
...
The GBM performs not so great on Body with an AUC of `r aucGBM$auc`.

The important variables are `r print(xtable(summary(gbmModel)), type="html")`
...
The BIC value of this is `r BIC(BodyEnvMS)` compare to `r BIC(BodyEnv)` for the one without variable selection.  It's AUC value is `r aucGLMMS$auc`. This really isn't much different than the other one with all of the variables. The AUC and AIC values are the same, and the BIC value only decreases because of parsimony (i.e. it penalizes having more variables).

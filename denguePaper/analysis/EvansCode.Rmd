...
m3 <- glmer(event ~ Class * block + (1|Tray_Code)+(1|Site_Code), 
                   data=allSurv,
                   family=binomial)

m4 <- glmer(event ~ Class + block + (1|Tray_Code)+(1|Site_Code), 
                   data=allSurv,
                   family=binomial)
...
survTray <- allSurv %>%
  ...
  summarise(survival=mean(event)*100)
...
siteSurv <- lmer(survival ~ Class * block + (1|Tray_Code)+(1|Site_Code), 
                   data=survTray)
...
#pairwise
summary(multcomp::glht(siteSurv, linfct = multcomp::mcp(block = "Tukey"), test = multcomp::adjusted("holm")))

summary(multcomp::glht(siteSurv, linfct = multcomp::mcp(Class = "Tukey"), test = multcomp::adjusted("holm")))
...
mixEmerge <- lmer(Exp_Day ~ Class * block + (1|Tray_Code)+(1|Site_Code), 
                   data=emergExpF)
summary(mixEmerge)
tidy(mixEmerge)
plot(mixEmerge)
...
#pairwise
summary(multcomp::glht(mixEmerge, linfct = mcp(block = "Tukey"), test = adjusted("holm")))

summary(multcomp::glht(mixEmerge, linfct = mcp(Class = "Tukey"), test = adjusted("holm")))

emergeGrid <-lsmeans::ref.grid(mixEmerge)
emergePair <- pairs(lsmeans(mixEmerge, ~Class|block))
lsmeans(mixEmerge, ~block)
```

...
mixModelseasonsBody21 <- lme4::glmer(Body~block + class + (1|site) , 
...
qqnorm(resid(mixModelseasonsBody21))

simulationOutput <- simulateResiduals(fittedModel = mixModelseasonsBody21)
plotSimulatedResiduals(simulationOutput = simulationOutput)
#plotResiduals(seasons$class[seasons$DPI=="21"],  simulationOutput$scaledResiduals)
```
...
#residuals
simulationOutput <- simulateResiduals(fittedModel = mixModelseasonsHead21)
plotSimulatedResiduals(simulationOutput = simulationOutput)
#plotResiduals(seasons$class[seasons$DPI=="21"],  simulationOutput$scaledResiduals)
```
...
```{r}
wingBody <- lmer(Body~Wing + (1|site), data=seasonInf)
wingBody <- glmer(Body~1 + sqrt(Wing) + (1|site), data=seasonInf, family=binomial(link='logit'))

overdisp_fun(wingBody) 
plot(wingBody)
summary(wingBody)
Anova(wingBody)

piecewiseSEM::rsquared(wingBody) #this RMSE is not so great
```

...
* plot KM curves and then analyze seasons seperately, using survival and emergence (or development rate)
...
km.fit <- survfit(Surv(Exp_Day, event) ~ block + Class +frailty(Site_Code), data=allSurv, type="kaplan-meier", se.fit=T, conf.int=0.95)
summary(km.fit)
...
```{r, eval=F}
coxFit <- survival::coxph(Surv(Exp_Day, event) ~ Class*block + block:Exp_Day, data=allSurv, method="breslow") #this doesn't converge, but I think is okay
summary(coxFit)

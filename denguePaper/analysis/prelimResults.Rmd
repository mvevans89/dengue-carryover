...
myCols <- c(respV, predVs, "block", "site")
modDF <- seasonInfSite %>%
...
$$ R_0(T) = \left (\frac{a(T)^2b(T)c(T)e^{-\mu (T)/PDR(T)}EFD(T)p_{EA}(T)MDR(T)} {Nr\mu(T)^3}  \right ) ^{1/2} $$

Bite rate, EIP, and adult survival will be calculated using rate summation from Mordecai et al 2017.  Vector competence, eggs per female per day, egg to adult survival probability and mosquito immature development rate are empirically measured. Human population density and recovery rate are held constant.
...
```{r}
N=100000 #human density
r=0.1 # human recovery rate
paramRate <- mutate(paramRate, 
                    R0=(((a^2)*bc*(exp(-mu/PDR))*EFD*pEA*(MDR^2))/(N*r*(mu^3)))^(1/2))
```

...
headWing <- glmer(Head~Wing + (1|block),
                          data=headWingDF,
                          family=binomial(link="logit"))
summary(headWing)
...
salWing <- glmer(Saliva~Wing + (1|block),
                          data=salWingDF,
                          family=binomial(link="logit"))
summary(salWing)
...
#curve(4.6320-1.8761*x, 1.5, 3, add=T, lwd=2)
#with(bodyWingDF, abline(lm(Body~Wing)))
ggplot(data=seasonInf, aes(x=factor(Saliva), y=Wing))+
  geom_violin()
...
finalBodyMix <- lmer(Body_mean~Temp_mean_mean + (1+Temp_mean_mean|block), data=modDF)
qqnorm(resid(finalBody))
car::Anova(finalBody)
summary(finalBody)
```
...
finalHead <- lm(Head_mean~ Temp_mean_mean , data=modDF)
Anova(finalHead)
plot(finalHead)
```

...
knitr::kable(Body21Aov, caption="Mixed model statistics for body level infections.")
knitr::kable(Head21Aov, caption="Mixed model statistics for head infections.")
```
...
#simulationOutput <- simulateResiduals(fittedModel = mixMortality)
#plotSimulatedResiduals(simulationOutput = simulationOutput)
#plotResiduals(as.factor(survSumm$block),  simulationOutput$scaledResiduals)
...
mixEmerge <- glm(Exp_Day ~  Class*block, #random effects competed with fixed effects (Hodges 2014)
                 data=emergExpF, 
...
summary(mixEmerge)
simulationOutput <- simulateResiduals(fittedModel = mixEmerge)
plotSimulatedResiduals(simulationOutput = simulationOutput)
plotResiduals(as.factor(emergExpF$Class),  simulationOutput$scaledResiduals)

knitr::kable(Anova(mixEmerge), caption = "Results from Mixed model on the effect of class and block on time to emergence.")

```

The day of emergence differed significantly across block and class. Mosquitoes in the summer emerged on average 14 days sooner than those in the fall. Rural mosquitoes emerged latest at rural sites in the fall, and latest at suburban sites in the summer.

Probably need to run some TukeyHSD business on this.

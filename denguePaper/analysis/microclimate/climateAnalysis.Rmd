...
Temperature Plot:
```{r, eval = F}
png(file="../figures/forMS/Figure2.png", width = 6, height=4, units = "in", res=400, family="sans")
...
Relative Humidity
```{r, eval=F}
#pdf(file="../figures/forMS/supplement/relativeHumiditySupp1.pdf", width = 6, height=4, family="sans")
...
```{r}
##------------ Mean T
climModMean <- lmer(meanT~Class*Block + (1|Site_ID/Day), data=climTray)
plot(climModMean)
summary(climModMean)
confint(climModMean)
...
intMod <- lmer(meanT~classxBlock+ (1|Site_ID/Day), data=climTray)
...
climModMin <- lmer(minT~Class*Block + (1|Site_ID/Day), data=climTray)
plot(climModMin)
summary(climModMin)
confint(climModMin)
AIC(climModMin)
...
##------------ Max T
climModMax <- lmer(maxT~Class*Block + (1|Site_ID/Day), data=climTray)
plot(climModMax)
summary(climModMax)
confint(climModMax)
...
##------------ DTR
climModDTR <- lmer(DTR~Class*Block + (1|Site_ID/Day), data=climTray)
plot(climModDTR)
summary(climModDTR)
confint(climModDTR)
...
#pairwise stats-interaction
intMod <- lmer(DTR~classxBlock+ (1|Site_ID/Day), data=climTray)
summary(glht(intMod, linfct = mcp(classxBlock = "Tukey"), test = adjusted("holm")))
...
climModMeanRH <- lmer(meanRH~Class*Block + (1|Site_ID/Day), data=climTray)
plot(climModMeanRH)
...
#pairwise stats-interaction
intMod <- lmer(meanRH~classxBlock+ (1|Site_ID/Day), data=climTray)
summary(glht(intMod, linfct = mcp(classxBlock = "Tukey"), test = adjusted("holm")))
...
climModMinRH <- lmer(minRH~Class*Block + (1|Site_ID/Day), data=climTray) #class not significant
plot(climModMinRH)
...
climModDHR <- lmer(DHR~Class*Block + (1|Site_ID/Day), data=climTray)
plot(climModDHR)
summary(climModDHR)
confint(climModDHR)
...
#pairwise stats-interaction
intMod <- lmer(DHR~classxBlock+ (1|Site_ID/Day), data=climTray)
summary(glht(intMod, linfct = mcp(classxBlock = "Tukey"), test = adjusted("holm")))
